...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('role', postgresql.ENUM('ADMIN', 'BANK_MANAGER', 'BANK_USER', 'DATA_PROVIDER', 'AUDITOR', 'CONSUMER', name='userrole', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('account_type', postgresql.ENUM('CREDIT_CARD', 'MORTGAGE', 'AUTO_LOAN', 'PERSONAL_LOAN', 'STUDENT_LOAN', 'LINE_OF_CREDIT', 'OTHER', name='accounttype', create_type=False), nullable=False),
        sa.Column('account_status', postgresql.ENUM('OPEN', 'CLOSED', 'DELINQUENT', 'CHARGE_OFF', 'COLLECTION', 'BANKRUPTCY', name='accountstatus', create_type=False), nullable=False),
        sa.Column('payment_status', postgresql.ENUM('CURRENT', 'LATE_30', 'LATE_60', 'LATE_90', 'LATE_120_PLUS', 'NO_PAYMENT', name='paymentstatus', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('purpose', postgresql.ENUM('LOAN_APPLICATION', 'CREDIT_CARD_APPLICATION', 'EMPLOYMENT', 'RENTAL_APPLICATION', 'INSURANCE', 'ACCOUNT_REVIEW', 'OTHER', name='inquirypurpose', create_type=False), nullable=False),
        ...
        sa.Column('status', postgresql.ENUM('PENDING', 'APPROVED', 'DENIED', 'CANCELLED', name='inquirystatus', create_type=False), server_default='PENDING', nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('reason', postgresql.ENUM('INCORRECT_BALANCE', 'INCORRECT_PAYMENT_HISTORY', 'ACCOUNT_NOT_MINE', 'DUPLICATE_ACCOUNT', 'FRAUD', 'IDENTITY_THEFT', 'OTHER', name='disputereason', create_type=False), nullable=False),
        sa.Column('description', sa.Text(), nullable=False),
        sa.Column('status', postgresql.ENUM('PENDING', 'UNDER_REVIEW', 'RESOLVED', 'REJECTED', 'WITHDRAWN', name='disputestatus', create_type=False), server_default='PENDING', nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('user_id', sa.Integer(), nullable=True),
        sa.Column('action', postgresql.ENUM('CREATE', 'READ', 'UPDATE', 'DELETE', 'LOGIN', 'LOGOUT', 'LOGIN_FAILED', 'PERMISSION_DENIED', 'DATA_EXPORT', 'PASSWORD_CHANGE', 'ACCOUNT_LOCKED', name='auditaction', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('consumer_id', sa.Integer(), nullable=False),
        sa.Column('consent_type', postgresql.ENUM('CREDIT_REPORT', 'DATA_SHARING', 'MARKETING', name='consenttype', create_type=False), nullable=False),
        sa.Column('status', postgresql.ENUM('GRANTED', 'REVOKED', 'EXPIRED', name='consentstatus', create_type=False), server_default='GRANTED', nullable=False),
        sa.Column('bank_id', sa.Integer(), nullable=True),
